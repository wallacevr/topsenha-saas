<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        Schema::create('senhas', function (Blueprint $table) {
            $table->id();
            $table->foreignId('fila_id')->constrained()->onDelete('cascade');
            $table->json('campos')->nullable(); // Campos dinâmicos
            $table->foreignId('guiche_id')->nullable()->constrained()->onDelete('set null');
            $table->string('senha'); // Ex: "AT-001"
            $table->boolean('prioritaria')->default(false);
            $table->enum('status', ['pendente', 'chamado', 'atendido'])->default('pendente');
            $table->timestamp('inicio_atendimento')->nullable();
            $table->timestamp('fim_atendimento')->nullable();
            $table->timestamps();
        });
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('senhas');
    }
};
